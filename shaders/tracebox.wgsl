...
fn computeProjectiveMain(@builtin(global_invocation_id) global_id: vec3u) {
  ...
  if (uv.x < texDim.x && uv.y < texDim.y) {
    ...
    // projective camera: rays start from origin (0,0,0) and point toward each pixel
    // the ray start point is always the origin
    var spt = vec3f(0, 0, 0);

    // calculate ray direction from origin to pixel
    // var rdir = vec3f(
    ...
    // smaller flocal length = wider field of view (object should be further)
    // larger focal length = narrow field of view (object should be closer)
