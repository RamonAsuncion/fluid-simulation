export default class GuiControls {
  ...
  _init(particles) {
    ...
    this.params = {
      ...
      switchMode: () => {
        const newMode = this.params.simulationMode;
    ...
    this.gui
      .add(this.params, "simulationMode", ["2D", "3D"])
      .name("Simulation Mode")
      .onChange(() => {
        this.params.switchMode();
      });
    ...
    particlesFolder
      ...
      .name("Show Marching Cubes") // Change name to be clearer
      .onChange((value) => {
        this.particles.setShowMarchingCubes(value); // Add this line to control marching cubes
        console.log(
          `Marching cubes visualization ${value ? "enabled" : "disabled"}`
        );
      });
    ...
    physicsFolder
      .add(this.params, "isoValue", 0, 5)
      .name("Isosurface Value")
      .onChange((value) => {
        this.particles.setIsoValue(value);
        console.log(`Iso Value changed to: ${value}`);
      });
