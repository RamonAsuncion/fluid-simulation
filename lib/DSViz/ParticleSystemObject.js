 ...
 *                                measures that legally restrict others from doidsfng
...
export default class ParticleSystemObject extends SceneObject {
  ...
  async createParticleGeometry() {
    ...
    this._time = new Float32Array([0, this._simulationSpeed]);
    this._timeBuffer = this._device.createBuffer({
      label: "time and speed buffer " + this.getName(),
  ...
  setSimulationSpeed(speed) {
    ...
    console.log(`Simulation speed set to ${this.simulationSpeed}`);
    const speedBuffer = new Float32Array([this.simulationSpeed]);
    this._device.queue.writeBuffer(
      this._timeBuffer,
      4, // offset
      speedBuffer
  ...
  togglePause() {
    ...
    console.log(`Simulation ${this._isPaused ? "paused" : "resumed"}`);
  }
  ...
  async createShaders() {
    ...
    this._bindGroupLayout = this._device.createBindGroupLayout({
      ...
      entries: [
        ...
        //   binding: 7,
        //   visibility: GPUShaderStage.COMPUTE,
        //   buffer: { type: "uniform" }, // mouse interaction buffer
        // },
      ],
    });
