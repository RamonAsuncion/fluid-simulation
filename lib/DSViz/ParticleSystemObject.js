...
export default class ParticleSystemObject extends SceneObject {
  ...
  async createParticleGeometry() {
    ...
    this._triangles = new Float32Array(Array(this._num_grid_cells * 15 * 3 * 3).fill(-1));
    this._triangleBuffer = this._device.createBuffer({
      label: "triangle buffer " + this.getName(),
  ...
  async createShaders() {
    ...
    if (this._triangleBuffer) {
      bindGroupEntries.push({
        binding: 10,
        visibility: GPUShaderStage.VERTEX | GPUShaderStage.COMPUTE,
        buffer: { type: "read-only-storage" }, // triangle buffer
      });
    } 
  ...
  async createParticlePipeline() {
    ...
    if (this._triangleBuffer) {
      bindGroupEntries1.push({
        binding: 10,
        resource: { buffer: this._triangleBuffer },
      });
      bindGroupEntries2.push({
        binding: 10,
        resource: { buffer: this._triangleBuffer },
