...
export default class ParticleSystemObject extends SceneObject {
  ...
  async createGeometry() {
    ...
    // Remove PGA3D specific code
    if (this._camera) {
      // Initialize with identity matrix
      const cameraData = new Float32Array(36);
      cameraData[0] = 1; // Identity matrix for init
  ...
  async createParticleGeometry() {
    ...
    console.log("new grid length: " + this._num_grid_cells, " ", this._max_per_cell)
  ...
  async fill_grid() {
    ...
    if (DEBUG) console.log("grid dimensions:", this._grid_length, this._grid_height, this._grid_width);
  }
  ...
  updateTimeBuffer() {
    console.log("array: ", 0, this._simulationSpeed, this._use_acceleration ? 1 : 0, this._r_value, this._g_value, this._b_value);
    const time = new Float32Array([0, this._simulationSpeed, this._use_acceleration ? 1 : 0, this._r_value, this._g_value, this._b_value]);
  ...
  updateCameraPose(camera) {
    if (this._cameraBuffer && camera) {
      ...
      // camera position
      /**
       * TODO: the axis are driving me nuts figure this out. there flipped.
       */
  ...
  updateBoundaryBox(width) {
    ...
    const new_num_grid_cells = new_grid_height * new_grid_length * new_grid_width;
