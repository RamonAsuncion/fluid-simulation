...
export default class ParticleSystemObject extends SceneObject {
  ...
  async createParticleGeometry() {
    ...
    this._triangles = new Float32Array(Array(this._num_grid_cells * 16 * 3 * 3).fill(-1));
    ...
    this._device.queue.writeBuffer(this._triangleBuffers[0], 0, this._triangles.buffer);
    this._device.queue.writeBuffer(this._triangleBuffers[1], 0, this._triangles.buffer);
  ...
  render(pass) {
    ...
    if (this._initialized) {
      ...
      // pass.draw(16, this._num_grid_cells);
    }
  }
