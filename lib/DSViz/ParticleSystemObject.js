...
export default class ParticleSystemObject extends SceneObject {
  constructor(device, canvasFormat, numParticles = 4096) {
    ...
    this._top_bound = 1;
    this._bot_bound = -1;
    this._grid_length = Math.ceil((Math.abs(this._left_bound) + Math.abs(this._right_bound)) / this._cell_size);
    this._grid_height = Math.ceil((Math.abs(this._top_bound) + Math.abs(this._bot_bound)) / this._cell_size);
  ...
  async createParticleGeometry() {
    ...
    this.resetParticles();
    this.fill_grid();
    // TODO: put particles in the grid and make sure it fits the maxpercell (at least for your first tick)
    this._gridBuffers = [
      this._device.createBuffer({
