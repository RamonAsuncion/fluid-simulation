...
export default class ParticleSystemObject extends SceneObject {
  constructor(device, canvasFormat, camera = null, numParticles = 4096) {
    ...
    this._mousePosition = new Float32Array([
      ...
      0,
      0,
    ]); // [x, y, isDown, radius, attractMode, forceMultiplier, padding, padding]
  }
  ...
  async createComputePipeline() {
    ...
    this._mousePipeline = this._device.createComputePipeline({
      label: "Mouse Compute Pipeline " + this.getName(),
      layout: this._pipelineLayout,
      compute: {
        module: this._shaderModule,
        entryPoint: "mouseRayMarch",
      },
    });
  }
  ...
  compute(pass) {
    if (this._initialized) {
      ...
      pass.setPipeline(this._mousePipeline); //perform mouse interaction
      pass.dispatchWorkgroups(this._num_particles);
