...
export default class ParticleSystemObject extends SceneObject {
  constructor(device, canvasFormat, numParticles = 4096) {
    ...
    const top_bound = 1;
    const bot_bound = -1;
    var grid_length = Math.ceil((Math.abs(left_bound) + Math.abs(right_bound)) / this._gridSize);
    var grid_height = Math.ceil((Math.abs(top_bound) + Math.abs(bot_bound)) / this._gridSize);
    this._numGridCells = grid_length * grid_height;
  }
  ...
  compute(pass) { 
    ...
    pass.dispatchWorkgroups(Math.ceil(this._numGridCells / 256));
